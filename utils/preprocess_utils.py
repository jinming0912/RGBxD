...
def random_flip(tensor_list, prob=0.5):
...
def randomly_scale_image_and_label(image, label=None, scale=1.0):
    ...
    if label is not None:
        label = tf.squeeze(tf.compat.v1.image.resize_nearest_neighbor(
    ...
    if label is not None:
        if label_layout_is_chw:
            ...
            resized_label = tf.compat.v1.image.resize_images(
                label, new_size, method=tf.compat.v1.image.ResizeMethod.NEAREST_NEIGHBOR,
